            ...
            <div className="p-4 pt-20 md:p-20 flex flex-col items-center max-w-7xl mx-auto justify-center border-x border-gray-200">
                ...
                <div className="flex flex-wrap gap-8 items-center justify-center mt-24">
                    {testimonials.map((testimonial, index) => (
                        <AnimatedContent delay={index * 0.10} key={index} className={`p-6 rounded-xl max-w-80 shadow-[0_4px_20px_rgba(0,0,0,0.10)] ${index === 1 ? 'bg-blue-600' : 'bg-white'}`}>
                            <div className="flex items-center gap-1">
                                {Array(testimonial.rating).fill(0).map((_, i) => (
                                    <StarIcon key={i} className={`size-5 ${index === 1 ? 'text-white fill-white' : 'fill-blue-500 text-yellow-500'}`} />
                                ))}
                            </div>
                ...
                <a href="#" className="mt-20 px-8 py-2.5 rounded-full bg-blue-50  border border-blue-200 text-yellow-500">
                    See more
                </a>
            </div>
        </section>
