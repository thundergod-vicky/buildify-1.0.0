...
export function Calendar({
  ...
  const [view, setView] = useState<"month" | "year" | "day" | "decade">(
    "month",
  );
  ...
  const fetchEvents = () => {
    ...
    api
      .get<any[]>(endpoint, auth.getToken() || "")
      .then((res) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                if (view === "decade")
                  setDecadeStart(new Date().getFullYear() - 4);
              }}
    ...
    return (
      <div className="grid grid-cols-7 border-b border-gray-100 bg-gray-50/50">
        {days.map((day) => (
          <div
            key={day}
            className="py-4 text-center text-[10px] font-black uppercase tracking-[0.2em] text-gray-400"
          >
            {day}
          </div>
        ))}
  ...
  const renderCells = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      ...
      cells.push(
        <div
          key={`prev-${i}`}
          className="p-4 h-32 border-r border-b border-gray-100 bg-gray-50/30 text-gray-300 pointer-events-none transition-all"
        >
    ...
    for (let i = 1; i <= remaining; i++) {
      cells.push(
        <div
          key={`next-${i}`}
          className="p-4 h-32 border-r border-b border-gray-100 bg-gray-50/30 text-gray-300 pointer-events-none"
        >
    ...
    return (
      <div className="grid grid-cols-7 overflow-hidden rounded-b-[2.5rem] border-l border-gray-100">
        {cells}
      </div>
    );
  };
    ...
    return (
      ...
      >
        <div className="p-6 sm:p-8 border-b border-gray-100 bg-gray-50/30">
          <h3 className="text-[10px] sm:text-sm font-black uppercase tracking-widest text-gray-400 mb-2">
            Schedule for
          </h3>
          ...
          {dayEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center px-6">
              ...
              <p className="text-xs text-gray-400 mt-2">
                Take some time to relax or catch up on your self-paced courses.
              </p>
            </div>
          ) : (
            dayEvents.map((event) => (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-gray-500 font-medium">
                    ...
                    {event.location ||
                      (event.isOnline ? "Online Classroom" : "Location TBA")}
                  </div>
                </div>
                  ...
                  <div className="mt-3 p-2 bg-blue-50/50 rounded-xl border border-blue-100/50 w-fit">
                    <div className="flex items-center gap-3 text-[9px] font-bold">
                      <div className="flex items-center gap-1 text-blue-600">
                        <span className="text-gray-400 font-black uppercase tracking-widest mr-1">
                          ID:
                        </span>
                ...
                <button
                  onClick={async () => {
                    ...
                    if (isPast && event.type === "CLASS") {
                      ...
                      try {
                        const res = await api.get<{
                          ...
                          `/class-sessions/${event.id}/recording`,
                          auth.getToken() || "",
                        );
                        if (
                          res &&
                          ...
                          if (streamRes.status === "processing") {
                            toast.info(
                              "Recording is still being processed. Please check back in a few minutes.",
                            );
                          ...
                          toast.info(
                            "Recording is not yet available. Please check again in a few minutes.",
                          );
                        }
                      } catch (e) {
                    ...
                    if (event.isOnline) {
                      ...
                      const token = auth.getToken();
                      window.open(
                        `${serverUrl}/class-sessions/attend/${event.id}${token ? `?token=${token}` : ""}`,
                        "_blank",
                      );
                    }
                  }}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isCurrent
                      ? "bg-blue-600 text-white border-transparent shadow-xl shadow-blue-200"
            ...
            {Array.from({ length: 12 }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setCurrentDate(
                      new Date(yearNum, currentDate.getMonth(), 1),
                    );
                    setView("year");
                  }}
                  className={cn(
                    ...
                    isSelected
                      ? "bg-blue-600 text-white border-transparent shadow-xl shadow-blue-200"
                      : isCurrent
                        ? "bg-yellow-50 text-yellow-600 border-blue-100"
                        : "bg-gray-50/50 border-transparent hover:bg-white hover:border-blue-100 hover:shadow-lg",
                  )}
                >
                  <span className="text-sm font-black uppercase tracking-widest">
          ...
          <div className="flex-1 flex items-center justify-center p-20 text-center">
            <div className="max-w-md">
              ...
              <p className="text-gray-400 font-medium">
                Select a date from the calendar to see detailed schedule and
                join classes.
              </p>
              <button
                onClick={() => setView("month")}
