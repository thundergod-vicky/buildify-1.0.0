...
import { Role, Batch, Subject, ClassSession } from "@/types";
...
export function BatchDetailsView({ batchId }: { batchId: string }) {
  ...
  const [activeTab, setActiveTab] = useState<"subjects" | "schedule" | "settings">("subjects");
  ...
  const isManagementRole = user?.role === Role.ADMIN || user?.role === Role.ACADEMIC_OPERATIONS;
  const isTeacher = user?.role === Role.TEACHER;

  if (loading) return (
    <div className="p-8 flex flex-col items-center justify-center min-h-[400px]">
      <div className="spinner scale-75 opacity-80"></div>
      <p className="text-gray-400 font-bold uppercase tracking-widest text-xs mt-4 animate-pulse">Syncing Batch Data...</p>
    </div>
  );
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-gray-900 font-urbanist tracking-tight">{batch.name}</h1>
          <p className="text-gray-500 font-medium mt-1">{batch.description || "Comprehensive academic batch management"}</p>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="flex -space-x-3">
            {batch.teachers?.slice(0, 3).map((t) => (
              <div key={t.id} className="size-10 rounded-full bg-blue-50 border-2 border-white flex items-center justify-center text-blue-600 font-bold text-sm" title={t.name}>
                {t.name?.[0]}
              </div>
            ))}
          ...
          <div className="text-right">
            <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Enrollment</p>
            <p className="text-sm font-black text-gray-900">{batch._count?.students || batch.students?.length || 0} Students</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 p-1.5 bg-gray-50 rounded-2xl w-fit">
        <button 
          ...
          className={cn(
            ...
            activeTab === "subjects" ? "bg-white text-blue-600 shadow-sm" : "text-gray-400 hover:text-gray-600"
          )}
        >
          Subjects
        </button>
        <button 
          ...
          className={cn(
            "px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
            activeTab === "schedule" ? "bg-white text-blue-600 shadow-sm" : "text-gray-400 hover:text-gray-600"
          )}
        >
          Schedule
        </button>
          ...
          <div className="space-y-6">
            {batch.subjects?.length === 0 ? (
              <div className="p-20 text-center bg-white rounded-[3rem] border border-gray-100 border-dashed">
                ...
                <h3 className="text-xl font-black text-gray-900">No Subjects Linked</h3>
                <p className="text-gray-400">Add subjects to this batch to organize your curriculum.</p>
                {isManagementRole && (
                   <button 
                    onClick={() => setActiveTab("settings")}
              ...
              batch.subjects?.map((subject) => (
                <div key={subject.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden group">
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-400 font-medium">Click to view classes and resources</p>
                      </div>
                    </div>
                    {expandedSubjects.includes(subject.id) ? <ChevronDownIcon className="size-5 text-gray-400" /> : <ChevronRightIcon className="size-5 text-gray-400" />}
                  </button>
                  ...
                  {expandedSubjects.includes(subject.id) && (
                    <div className="px-6 pb-6 pt-2 space-y-4">
                      ...
                      <div className="space-y-4">
                        ...
                        {batch.sessions?.filter(s => s.subjectId === subject.id).length === 0 ? (
                          <p className="text-sm text-gray-400 italic py-4">No classes scheduled for this subject yet.</p>
                        ) : (
                          batch.sessions?.filter(s => s.subjectId === subject.id).map((session) => (
                            <ClassItem 
                              key={session.id} 
                              session={session} 
           ...
           <div className="space-y-6">
             {batch.sessions?.length === 0 ? (
               <div className="p-20 text-center bg-white rounded-[3rem] border border-gray-100 border-dashed">
                 ...
                 <p className="text-gray-400">Synchronize your routine from the class manager.</p>
               </div>
             ) : (
              batch.sessions?.map((session) => (
                <ClassItem key={session.id} session={session} isManagementRole={isManagementRole} isTeacher={isTeacher && session.teacherId === user?.id} />
              ))
             )}
           </div>
        )}
...
function ClassItem({ session, isManagementRole, isTeacher, onRefresh }: { session: ClassSession, isManagementRole: boolean, isTeacher: boolean, onRefresh: () => void }) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <div className="size-16 bg-white rounded-2xl flex flex-col items-center justify-center border border-gray-100 shadow-sm">
            <span className="text-[10px] font-black uppercase text-blue-600">{sessionDate.toLocaleDateString('en-US', { month: 'short' })}</span>
            <span className="text-xl font-black text-gray-900">{sessionDate.getDate()}</span>
          </div>
          <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {(isManagementRole || isTeacher) && (
                  <div className="flex items-center gap-2">
                    {session.meetingId && (
                      <button 
                        onClick={async (e) => {
                          ...
                          try {
                            await api.get(`/class-sessions/${session.id}/recording`, auth.getToken()!);
                            toast.update(loadId, { render: "Sync complete! Please refresh if recordings do not appear.", type: "success", isLoading: false, autoClose: 3000 });
                            onRefresh();
                          } catch (err: any) {
                            toast.update(loadId, { render: err.message || "Failed to sync", type: "error", isLoading: false, autoClose: 3000 });
                          }
                        }}
                    ...
                    <button className="text-blue-600 hover:text-blue-700 p-1" title="Add manually">
              ...
              <div className="space-y-2">
                {session.recordings?.length === 0 ? (
                  <p className="text-xs text-gray-400 italic">No recordings uploaded for this session.</p>
                ) : (
                  session.recordings?.map(rec => (
                    <div key={rec.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl group/item">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {rec.passcode && <p className="text-[10px] text-gray-400">Passcode: {rec.passcode}</p>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <a href={rec.url} target="_blank" className="p-2 text-gray-400 hover:text-blue-600 transition-colors">
            ...
            <div className="space-y-4">
                  ...
                  <label className="text-blue-600 hover:text-blue-700 p-1 cursor-pointer">
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        const loadId = toast.loading("Uploading study material...");
                        try {
                          // Upload to S3 via backend
                          const uploadRes = await api.upload<{ id: string; url: string }>('/content/upload', file, auth.getToken()!);
                          
                          let type = 'OTHER';
                          const nameLower = file.name.toLowerCase();
                          if (nameLower.endsWith('.pdf')) type = 'PDF';
                          else if (nameLower.endsWith('.ppt') || nameLower.endsWith('.pptx')) type = 'PRESENTATION';
                          
                          // Link to session
                          await api.post(`/class-sessions/${session.id}/attachments`, {
                            title: file.name,
                            url: uploadRes.url,
                            type
                          }, auth.getToken()!);
                          
                          toast.update(loadId, { render: "Upload complete!", type: "success", isLoading: false, autoClose: 2000 });
                          e.target.value = ''; // Reset input
                          onRefresh();
                        } catch (err: any) {
                          toast.update(loadId, { render: err.message || "Failed to upload", type: "error", isLoading: false, autoClose: 3000 });
                        }
                      }}
                    />
                    <PlusCircleIcon className="size-4" />
              ...
              <div className="space-y-2">
                 {session.attachments?.length === 0 ? (
                  <p className="text-xs text-gray-400 italic">No notes or presentations available.</p>
                ) : (
                  session.attachments?.map(att => (
                    <div key={att.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl group/item">
                      <div className="flex items-center gap-3">
                        <div className="size-8 bg-amber-50 text-amber-600 rounded-lg flex items-center justify-center font-black text-[10px]">
                          {att.type.slice(0, 3)}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-gray-700">{att.title}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <a href={att.url} target="_blank" className="p-2 text-gray-400 hover:text-blue-600 transition-colors">
                          <ExternalLinkIcon className="size-4" />
                        </a>
                        {(isManagementRole || isTeacher) && (
                          <button className="p-2 text-gray-400 hover:text-red-600 transition-colors">
...
function BatchSettings({ batch, onUpdate }: { batch: Batch, onUpdate: () => void }) {
  ...
  const handleAddSubject = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post(`/batches/${batch.id}/subjects`, { name: newSubject }, auth.getToken()!);
  ...
  const handleRemoveSubject = async (id: string) => {
    if (!confirm("Are you sure? Removing a subject will un-link classes.")) return;
    try {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Add or remove subjects for this batch</p>
        </div>
        ...
        <div className="space-y-2">
          {batch.subjects?.map(subject => (
            <div key={subject.id} className="flex items-center justify-between p-4 bg-gray-50/50 border border-gray-100 rounded-2xl group">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Overview of students and teachers</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-blue-50/50 rounded-2xl">
            <div className="flex items-center gap-3">
              ...
              <span className="font-black text-gray-900 text-sm">Total Students</span>
            </div>
            <span className="font-black text-blue-600">{batch._count?.students || batch.students?.length || 0}</span>
          </div>
          
          <div className="space-y-2">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Assigned Teachers</p>
             {batch.teachers?.map(teacher => (
               <div key={teacher.id} className="flex items-center gap-3 p-3 border border-gray-100 rounded-2xl">
                 <div className="size-8 bg-gray-100 rounded-full flex items-center justify-center font-bold text-xs text-gray-500">
                    {teacher.name?.[0]}
                 </div>
                 <span className="text-sm font-bold text-gray-700">{teacher.name}</span>
               </div>
             ))}
          </div>
        </div>
