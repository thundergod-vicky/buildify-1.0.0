...
import { ArrowLeftIcon, UploadIcon, FileTextIcon, Loader2Icon, CheckCircleIcon, XCircleIcon, CameraIcon, EyeIcon, XIcon } from "lucide-react";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] min-h-0 overflow-hidden flex flex-col shadow-2xl">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 font-urbanist">{selectedResult.studentName || "Result"} Details</h2>
                <p className="text-gray-500 text-sm">Comparison with {template.name}</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4 min-h-0">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Student OMR Image</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Question Breakdown</h3>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {template.answers.map((correction: { number: number; answer: string }) => {
                  const studentAns = selectedResult.answers.find((a: { number: number; answer: string }) => a.number === correction.number);
                  const isCorrect = studentAns?.answer === correction.answer;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`absolute top-0 left-0 w-full h-1 ${isCorrect ? 'bg-green-400' : 'bg-red-400'}`} />
                      
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-black text-gray-400 uppercase tracking-tighter">Question {correction.number}</span>
                        {isCorrect ? (
                      ...
                      <div className="flex items-start justify-between gap-2 border-t border-gray-100/50 pt-2 mt-auto">
                        <div className="flex-1 min-w-0">
                          <p className={`text-[9px] font-bold uppercase tracking-tight truncate ${isCorrect ? 'text-green-500' : 'text-red-400'}`}>
                            Student
                          </p>
                          <p className={`text-2xl font-black font-urbanist leading-none ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                            {studentAns?.answer || "-"}
                          </p>
                        </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-4">
                 <div className="text-right border-r pr-4 border-gray-200">
                    <p className="text-sm text-gray-500 font-bold uppercase tracking-widest">Final Score</p>
                    <p className="text-3xl font-black text-gray-900">{selectedResult.score}<span className="text-lg text-gray-400">/{selectedResult.total}</span></p>
                 </div>
