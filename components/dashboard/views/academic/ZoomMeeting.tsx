...
export function ZoomMeeting() {
  ...
  const initialError = !meetingNumber
    ? "Meeting ID is missing from the URL."
    : null;
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (type === "ZOOM_FRAME_READY") {
        ...
        if (status === "joined") {
          console.log(
            "[Zoom SDK] Frame ready but already joined. Skipping start.",
          );
          return;
        }
        ...
        try {
          ...
          const cleanMeetingNumber = (meetingNumber || "").replace(
            /[^0-9]/g,
            "",
          );
          ...
          setError(
            e.reason || e.message || "Failed to fetch meeting credentials.",
          );
          setStatus("error");
        }
      }
    ...
    <div className="fixed inset-0 z-[99999] bg-black overflow-hidden flex flex-col animate-in fade-in duration-700">
      ...
      <div className="p-6 pb-2 flex items-center justify-between bg-gradient-to-b from-black/90 to-black/40 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-sm font-black tracking-[0.2em] uppercase opacity-90">
              Virtual Classroom
            </h1>
